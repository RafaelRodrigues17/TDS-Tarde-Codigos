...
public class TeatroMagico {
    ...
    //Vende um ingresso altera o status para vendido e registra no log
    public static boolean venderIngresso(String[][][] teatro, int andar, int coluna, int linha) {
        if (andar < 1 || andar > 3) {
            Erro.andarInvalido();
            return false;
        }
        if (coluna < 1 || coluna > teatro[andar - 1].length) {
            Erro.colunaInvalida();
            return false;
        }
        if (linha < 1 || linha > teatro[andar - 1][0].length) {
            Erro.linhaInvalida();
            return false;
        }
    ...
    //Salva o log com a data hora status
