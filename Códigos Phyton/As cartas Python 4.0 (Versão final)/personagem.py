...
class Personagem:
    ...
    def comprar_carta(self, baralho): 
        carta_comprada = random.choice(baralho)
        self.mao_de_cartas.append(carta_comprada)
        return f"{self.nome} comprou a carta {carta_comprada.nome}."
    
    def usar_carta(self, carta, alvo: Personagem = None):
        if carta in self.mao_de_cartas:
            resultado = carta.usar_carta(self, alvo)
            self.mao_de_cartas.remove(carta)
            return resultado
        return "Carta não está na mão."
    
    def exibir_cartas(self):
        return self.mao_de_cartas
    
    def curar(self, pontos_de_vida):
